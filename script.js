...
(function () {
  ...
  function initSearch() {
    ...
    input.addEventListener("input", () => {
      ...
      // FIX: tanda kutip + toggle (bukan selalu remove)
  ...
  /* =========================================================
     5. FAVORIT & VIDEO (EVENT DELEGATION)
     ========================================================= */
  ...
  function openVideoModal(productId) {
    ...
    if (vid.type === "youtube") {
      // FIX: separator dinamis supaya tidak salah gabung
      const sep = vid.embed.includes("?") ? "&" : "?";
      mediaHTML = `
        <iframe
          src="${vid.embed}${sep}autoplay=1"
      ...
      // FIX: tipe drive ditangani terpisah, tanpa autoplay (Drive tidak dukung)
      mediaHTML = `
        <iframe
          src="${escapeHtml(vid.embed)}"
          allow="autoplay"
          allowfullscreen
          frameborder="0"
          title="${escapeHtml(product.name)}">
      ...
      mediaHTML = `
        <div style="display:grid;place-items:center;width:100%;height:100%;color:#fff;padding:20px;text-align:center;">
  ...
  function initProfileLinks() {
    ...
    if (contactGrid && Array.isArray(PROFILE.kontak)) {
      contactGrid.innerHTML = PROFILE.kontak
        .map(
          (c) => `
  ...
  /* =========================================================
     10. FILTER & SORTIR (HALAMAN KATALOG)
     ========================================================= */
  ...
  function applyFilters() {
    ...
    if (state.badgeFilter) {
      list = list.filter(
        (p) => (p.badge || "").toUpperCase() === state.badgeFilter
      );
    }
  ...
  function readURLParams() {
    ...
    if (cat) {
      state.category =
        CATEGORIES.find((c) => c.name.toLowerCase() === cat.toLowerCase())?.name ||
        cat;
    }
  ...
  function renderHome() {
    ...
    if (terlaris.length < 4) {
      terlaris = terlaris.concat(all.filter((p) => p.badge !== "TERLARIS"));
    }
